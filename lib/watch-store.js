...
class WatchStore {
  constructor(kernel, outputService) {
    ...
    // newline (see keymaps).
    this._confirmCommand = lumine.commands.add(this.editor.element, {
      "core:confirm": () => {
        if (!this.isWatching) {
          this.toggleWatching();
        } else {
  ...
  run = () => {
    ...
    // than the expression evaluates. One run at a time: a tick that arrives
    // mid-run is dropped, and the next idle runs again — re-running then would
    // only duplicate the value and burn the scrubbable history.
    ...
    if (code && code.length > 0) {
      ...
      this.kernel.executeWatch(code, (result) => {
        // An error settles the run too — a restart or a dead process answers
        // with one, and a transport that never follows it with an idle must
        // not leave the watch latched for the life of the window.
        if (
